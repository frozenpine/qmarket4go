...
extern "C" {
	...
	typedef void (APPWINAPI *FunOnFrontConnected)(ClientID client);//回调函数
	...
	///        0x2002 发送心跳失败
	///        0x2003 收到错误报文
	typedef void (APPWINAPI *FunOnFrontDisconnected)(ClientID client, int nReason);
	MDUSER_API_EXPORT int SetCallbackFunOnFrontDisconnected(ClientID client,FunOnFrontDisconnected funHandle);
	
	///心跳超时警告。当长时间未收到报文时，该方法被调用。
	///@param nTimeLapse 距离上次接收报文的时间
	typedef void (APPWINAPI *FunOnHeartBeatWarning)(ClientID client, int nTimeLapse);
	...
	typedef void (APPWINAPI *FunOnPackageStart)(ClientID client, int nTopicID, int nSequenceNo);
	MDUSER_API_EXPORT int SetCallbackFunOnPackageStart(ClientID client,FunOnPackageStart funHandle);
	
	///报文回调结束通知。当API收到一个报文后，首先调用报文回调开始通知，然后是各数据域的回调，最后调用本方法。
	///@param nTopicID 主题代码（如私有流、公共流、行情流等）
	///@param nSequenceNo 报文序号
	typedef void (APPWINAPI *FunOnPackageEnd)(ClientID client, int nTopicID, int nSequenceNo);
	MDUSER_API_EXPORT int SetCallbackFunOnPackageEnd(ClientID client,FunOnPackageEnd funHandle);

    ///新增多播心跳接口
	typedef void (APPWINAPI *FunOnMultiHeartbeat)(ClientID client, char *CurrTime,char *MultiCastIP);
    MDUSER_API_EXPORT int SetCallbackFunOnMultiHeartbeat(ClientID client,FunOnMultiHeartbeat funHandle);
	
	///订阅多播主题
	typedef void (APPWINAPI *FunOnStopMultiTopic)(ClientID client, int nTopicID);
    MDUSER_API_EXPORT int SetCallbackFunOnStopMultiTopic(ClientID client,FunOnStopMultiTopic funHandle);
	
    typedef void (APPWINAPI *FunUdpMarketData)(ClientID client, CQdamFtdcDepthMarketDataField *qmdata);
	MDUSER_API_EXPORT int SetCallbackFunUdpMarketData(ClientID client,FunUdpMarketData funHandle);
	///错误应答
	typedef void (APPWINAPI *FunOnRspError)(ClientID client, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspError(ClientID client,FunOnRspError funHandle);
	///用户登录应答
	typedef void (APPWINAPI *FunOnRspUserLogin)(ClientID client, CQdamFtdcRspUserLoginField *pRspUserLogin, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspUserLogin(ClientID client,FunOnRspUserLogin funHandle);
	///用户退出应答
	typedef void (APPWINAPI *FunOnRspUserLogout)(ClientID client, CQdamFtdcRspUserLogoutField *pRspUserLogout, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspUserLogout(ClientID client,FunOnRspUserLogout funHandle);
	///深度行情通知
	typedef void (APPWINAPI *FunOnRtnDepthMarketData)(ClientID client, CQdamFtdcDepthMarketDataField *pDepthMarketData);
	MDUSER_API_EXPORT int SetCallbackFunOnRtnDepthMarketData(ClientID client,FunOnRtnDepthMarketData funHandle);
	///深度行情通知
	typedef void (APPWINAPI *FunOnRtnMultiDepthMarketData)(ClientID client, CQdamFtdcDepthMarketDataField *pDepthMarketData);
	MDUSER_API_EXPORT int SetCallbackFunOnRtnMultiDepthMarketData(ClientID client,FunOnRtnMultiDepthMarketData funHandle);
	///订阅合约的相关信息
	typedef void (APPWINAPI *FunOnRspSubMarketData)(ClientID client, CQdamFtdcSpecificInstrumentField *pSpecificInstrument, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspSubMarketData(ClientID client,FunOnRspSubMarketData funHandle);
	///退订合约的相关信息
	typedef void (APPWINAPI *FunOnRspUnSubMarketData)(ClientID client, CQdamFtdcSpecificInstrumentField *pSpecificInstrument, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspUnSubMarketData(ClientID client,FunOnRspUnSubMarketData funHandle);
	///分价行情
	typedef void (APPWINAPI *FunOnRtnMBLMarketData)(ClientID client, CQdamFtdcMBLMarketDataField *pMBLMarketData);
	MDUSER_API_EXPORT int SetCallbackFunOnRtnMBLMarketData(ClientID client,FunOnRtnMBLMarketData funHandle);
	///郑州合约状态
	typedef void (APPWINAPI *FunOnRtnQmdInstrumentStatu)(ClientID client, CQdamFtdcQmdInstrumentStateField *pQmdInstrumentState);
	MDUSER_API_EXPORT int SetCallbackFunOnRtnQmdInstrumentStatu(ClientID client,FunOnRtnQmdInstrumentStatu funHandle);
	///订阅主题应答
	typedef void (APPWINAPI *FunOnRspSubscribeTopic)(ClientID client, CQdamFtdcDisseminationField *pDissemination, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspSubscribeTopic(ClientID client,FunOnRspSubscribeTopic funHandle);
	///主题查询应答
	typedef void (APPWINAPI *FunOnRspQryTopic)(ClientID client, CQdamFtdcDisseminationField *pDissemination, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
	MDUSER_API_EXPORT int SetCallbackFunOnRspQryTopic(ClientID client,FunOnRspQryTopic funHandle);
	///合约行情应答
	typedef void (APPWINAPI *FunOnRspQryMarketData)(ClientID client, CQdamFtdcRspMarketDataField *pRspMarketData, CQdamFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast);
